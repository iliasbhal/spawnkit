...
describe("Instance", () => {
	...
	it("should save the snapshot to the database", async () => {
		...
		const storedSnapshot: any = await InstanceSnapshot.getActorSnapshot(actorInst.id);
	...
	it.todo("should not emit a new snaphot when actor is reconstrctued with a snapshot");
});
